...
// Función para obtener información de recargo por transferencia bancaria de un producto
export const getProductBankTransferInfo = (product) => {
  ...
  if (savedConfig) {
    try {
      ...
      if (bankConfig && bankConfig.isEnabled) {
        // Obtener recargo específico para la categoría del producto
        const categorySpecificSurcharge = bankConfig.categorySpecificSurcharges[product.category];
        const productSurcharge = categorySpecificSurcharge !== undefined 
          ? categorySpecificSurcharge 
          : bankConfig.defaultSurcharge;
        ...
        return {
          isEnabled: true,
          surchargePercent: productSurcharge,
          surchargeAmount: surchargeAmount,
          totalWithSurcharge: totalWithSurcharge,
          isCustomSurcharge: categorySpecificSurcharge !== undefined
        };
      }
    } catch (error) {
      console.error('Error al obtener información de transferencia bancaria:', error);
    }
  }
  ...
  return {
    ...
    surchargePercent: 0,
    surchargeAmount: 0,
    totalWithSurcharge: product.price,
    isCustomSurcharge: false
  };
};